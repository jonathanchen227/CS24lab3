...
class IntBST {
 ...
 public:
    ...
    int count() const;               // count of values // true if value is in tree
 ...
 private:
    ...
    bool remove(int value, Node*& n);
    bool contains(int value, Node* n) const;// these should be used by getPredecessor and getSuccessor, and ONE of them should be used by remove
    int getSuccessor(int value,Node* n) const;   // returns the Node containing the successor of the given value
    int getPredecessor(int value,Node* n) const; // returns the Node containing the predecessor of the given value 
};
